...
// URLの末尾に紛れ込みやすい、Markdownの区切り文字(リンクの閉じ括弧`)`、強調の`_`など)を
// 取り除く。例: "[Claude Code](https://claude.ai/code)_" から抽出すると、URLパターンの
// \S+ が閉じ括弧と強調記号まで含んでしまうため。
const TRAILING_MARKDOWN_NOISE_PATTERN = /[)\]}>_,.!?'"]+$/;
// Claude Codeが自動でコメント末尾に付与するattributionフッター
// (`_Generated by [Claude Code](https://claude.ai/code)_`)に含まれるリンクは、
// 運用者が貼った「記事の公開先URL」では有り得ないため、公開先URL候補から除外する。
const EXCLUDED_PUBLISHED_URL_HOSTS = ["claude.ai", "claude.com", "anthropic.com"];

function isExcludedPublishedUrlHost(url: string): boolean {
  let host: string;
  try {
    host = new URL(url).hostname;
  } catch {
    return true; // 不正なURLは公開先URLとして扱わない
  }
  return EXCLUDED_PUBLISHED_URL_HOSTS.some(
    (excluded) => host === excluded || host.endsWith(`.${excluded}`)
  );
}

// 与えられたコメント本文(古い順)から、運用者が貼った記事の公開先URLを抜き出す。
// 最新のコメントから遡り、Claude Codeのattributionフッターなど公開先URLとして
// あり得ないリンクは無視して、最初に見つかった有効なURLを返す。
export function extractPublishedUrlFromComments(commentBodies: string[]): string | null {
  for (let i = commentBodies.length - 1; i >= 0; i--) {
    const match = commentBodies[i].match(URL_PATTERN);
    if (!match) continue;
    const url = match[0].replace(TRAILING_MARKDOWN_NOISE_PATTERN, "");
    if (url && !isExcludedPublishedUrlHost(url)) return url;
  }
  return null;
}

// 記事issueのコメントからURLを抜き出す。運用者が「記事を公開したら最後のコメントに
...
export async function getLastCommentUrl(owner: string, repo: string, issueNumber: number): Promise<string | null> {
  ...
  return extractPublishedUrlFromComments(comments.map((c) => c.body));
}
