...
// 最新のコメントから遡り、isAllowedを満たす最初のURLを返す。記事issueには
// Claude Codeのattributionフッター(claude.aiへのリンク)など、公開先ではないURLも
// 書き込まれるため、除外リストではなく許可するURLの形で絞り込む。
export function extractPublishedUrlFromComments(
  commentBodies: string[],
  isAllowed: (url: string) => boolean
): string | null {
  for (let i = commentBodies.length - 1; i >= 0; i--) {
    for (const match of commentBodies[i].matchAll(URL_PATTERN)) {
      const url = match[0].replace(TRAILING_MARKDOWN_NOISE_PATTERN, "");
      if (url && isAllowed(url)) return url;
    }
  }
  return null;
}

// 記事issueのコメントから、運用者が貼った記事の公開先URLを抜き出す(X投稿生成が、
// 公開済み記事のURLをここから拾う)。直近100件のコメントまでしか見ないため、それより
// 古いコメントにしかURLが無い場合は拾えない(通常の運用では起こらない想定)。
export async function getLastCommentUrl(
  owner: string,
  repo: string,
  issueNumber: number,
  isAllowed: (url: string) => boolean
  ...
  return extractPublishedUrlFromComments(
