...
- URLやリンクが一切含まれていないか。リプライ本文にリンクは不要です。特に、
  このプロンプトを実行しているClaude.aiのチャット自体の共有リンク(claude.ai/...)を
  誤って含めていないか、必ず確認してください。
- 全角${input.charLimit}文字を絶対に超えていないか。目安は全角60〜120文字程度。

# 出力

- 上のチェック項目をすべて満たす、最終版のリプライ本文のみを出力してください。
  前置き・説明・引用符・コードブロック記法・URL・リンクは一切含めないでください。
...
- URLやリンクが一切含まれていないか。特に、このチャット自体の共有リンク(claude.ai/...)を
  貼り付ける際に誤って含めていないか、必ず確認してください。含まれていた場合は必ず削除して
  ください。
...
ください。URLやリンクは最終版に含めないでください。問題が無ければ元の文をそのまま
最終版としてください。`;
}
