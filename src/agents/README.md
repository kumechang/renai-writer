...
`--issue` で指定したissueだけでなく、保留中のプロンプトを持つ全issueをまとめて処理する
(結果は指定issue分を先頭にした複数行で返る)。これはGitHub Actions側の
`concurrency`グループ(bot.dbへの書き込み競合を避けるため、記事関連の全ワークフローで
`console-state`を共有している)の仕様上、あるissueへの返信をトリガーにした実行が
別のイベントに追い越されて「pendingのまま一度も実行されずキャンセル」されることが
あるための対策。そのissue分の処理が取りこぼされても、次にどのissueであれ`check`が
実際に実行されたタイミングで一緒に拾われる。
...
- 返信の解析は、コメント本文から最初の \`\`\`json ... \`\`\` コードブロックを取り出して
  ...
  Claudeの回答が長文の記事本文を含む場合、強調のためのASCII二重引用符がエスケープ
  されずJSONとして壊れることがあった（実例: issue #5）。長文を貼り直させるコストが
  高いため、`JSON.parse` が失敗した場合は `jsonrepair` による自動修復を1回だけ試み、
  修復結果がオブジェクトであればそれを採用する（修復してもオブジェクトにならない
  場合は元のパースエラーで失敗させる）。
